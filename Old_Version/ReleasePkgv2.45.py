...
def Ftpdownload():# If Fv folder and .zip can't find, Download from HP FTP
	Ftpfilelist=[]
	for Proc in ProcSplit:# Find process BoardID
		Ftp.cwd("/ProductionReleaseDT/"+Proc[2]+"_"+Proc[0])
		Ftp.dir(Ftpfilelist.append)# Write FTP dir to Ftpfilelist
		if NewBuildID=="" or NewBuildID=="0000":
			Ftpfilelist=[File for File in Ftpfilelist if "Fv_"+Proc[2]+"_"+NewVersion+"_0000_BuildJob_" in File]
			if len(Ftpfilelist)==0:
				print("Download "+"Fv_"+Proc[2]+"_"+NewVersion+"_0000_BuildJob"+" Failed, Is not in the ftp site.")
				continue
		else:
			Ftpfilelist=[File for File in Ftpfilelist if "Fv_"+Proc[2]+"_"+NewVersion+"_"+NewBuildID+"_BuildJob_" in File]
			if len(Ftpfilelist)==0:
				print("Download "+"Fv_"+Proc[2]+"_"+NewVersion+"_"+NewBuildID+"_BuildJob"+" Failed, Is not in the ftp site.")
				continue
		Data=Ftpfilelist[0].split("Fv_");Data="Fv_"+Data[1]# Set Fv file full name
		if not os.path.isfile(CWD+"\\"+Data) and len(Matchfolderlist)==0:# Check Fv zip exists
			print("Start Download "+Data+" from FTP, Please wait.....")
			f=open(Data, "wb")
			try:Ftp.retrbinary('RETR ' + Data, f.write)# Start download Fv file
			except ftplib.error_perm:	print("Download "+Data+" Failed, Is not in the ftp site.")
			print("Download "+Data+" succeeded.")
			f.close()
			Matchziplist.append(Data)
		elif not os.path.isfile(CWD+"\\"+Data) and not Data.split(".")[0] in Matchfolderlist:# If Fv folder not match Projectlist
			print("Start Download "+Data+" from FTP, Please wait.....")
			f=open(Data, "wb")
			try:Ftp.retrbinary("RETR " + Data, f.write)# Start download Fv file
			except ftplib.error_perm:	print("Download "+Data+" Failed, Is not in the ftp site.")
...
NewProcSplit=ProcSplit.copy()# New Pkg Name List
if NewBuildID=="" or NewBuildID=="0000":
