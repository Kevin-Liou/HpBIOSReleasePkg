...
def Ftp_callback(chunk, file_size, dst_file):
	...
	Ftp_callback.progress_size+=len(chunk)# chunk is size of data received each time
	...
	for Proc in ProcSplit:# Find process BoardID
		...
		if not os.path.isfile(CWD+"\\"+Data) and len(Matchfolderlist)==0:# Check Fv zip exists
			...
			try:# Start download Fv file, and callback chunk size
				Ftp.retrbinary("RETR %s/%s" % (Ftp_dir, Data), lambda chunk:Ftp_callback(chunk, File_size, f))
