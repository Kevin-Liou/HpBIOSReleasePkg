...
if __name__ == '__main__':
    ...
    #=================Script Start==========================================================
    print("Make Release Pkg Script   4.0.1".center(90, "="))
    ...
    OldVersion_AMD = OldVersion[0:2]+"."+OldVersion[2:4]+"."+OldVersion[4:6]
    print("Start Find Process Project Old Pkg.")
    for Project in ProcessProjectList:
        temp=[]
        #======For Intel
        if Platform_Flag(Project) == "Intel":
    ...
    for OldProcPkg in OldProcPkgInfo:
        #======For Intel
        if Platform_Flag(OldProcPkg) == "Intel":
        ...
        #======For AMD
            ...
            for Proc in range(len(NeedProcOldPkg)):
                NewVersion_AMD = NewVersion[0:2]+"."+NewVersion[2:4]+"."+NewVersion[4:6]
    ...
    for NProc in NewProcPkgInfo:# Pkg Modify Update Version
        #======For Intel
        if Platform_Flag(NProc) == "Intel":
            Path=os.getcwd()+"\\"+("_").join(NProc)
            if os.path.isdir(Path+"\\FPTW"):# Check Folder Exist
                ReleaseNote_docx = [ReleaseNote for ReleaseNote in os.listdir(Path) if "release note.docx" in ReleaseNote]
                ReleaseNote_xlsm = [ReleaseNote for ReleaseNote in os.listdir(Path) if "Release_Note.xlsm" in ReleaseNote]
                if len(ReleaseNote_docx)==1:
                    ...
                    os.chdir(Path)
                    os.rename(ReleaseNote_xlsm[0], ("_").join(NProc) + "_BIOS_Release_Note.xlsm")
                    if os.path.isfile(("_").join(NProc) + "_BIOS_Release_Note.xlsm"):
                        print("ReleaseNote Rename to " + ("_").join(NProc) + "_BIOS_Release_Note.xlsm" + " succeeded.")
                else:
                    print("Can't find release note file.")
                os.chdir(Path+"\\FPTW")
                ChangeBuildID(NProc, Versionfilelist, NewVersion)
            else:   print("Pkg Folder "+("_").join(NProc)+" can't find.\n")
            os.chdir("..\..")
        #======For AMD
        else:
            Path=os.getcwd()+"\\"+("_").join(NProc)
            if os.path.isdir(Path+"\\AMDFLASH"):# Check Folder Exist
                ...
                ChangeBuildID_AMD(NProc, Versionfilelist_AMD, NewVersion)
            ...
            if Platform_Flag(NProc) == "Intel":
                if Fv.split("_")[1]==NProc[2]:
                    if os.path.isdir(".\\"+Fv):
                        ...
                        if os.path.isdir(Path + "\\Combined\\WU") and os.path.isdir(Path + "\\Combined\\FUR"):
                            New_FvFolder_Move_File(Path)# For Intel G5 Combined folder
    ...
    print("End".center(90, "="))
    #======For Intel Check
    if Platform_Flag(OldProcPkgInfo) == "Intel":
        CheckPkg(NewProcPkgInfo, NewVersion)# Check new release Pkg is OK?
        CheckFileCRC32(NewProcPkgInfo, NewVersion)
    #======For AMD Check
    else:
