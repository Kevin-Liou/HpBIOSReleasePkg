#=================================
# coding=UTF-8
# Make Release Pkg Script.   4.3.9
# Author: Kevin Liou
# Contact: Kevin.Liou@quantatw.com

#This script is for makeing release pkg
#Now can use Intel G4, G5, G6 AMD G4 platform
#=================================

from Lib import *

# In add the Projects to be here, and the following BoradID should also add.
BoardID=["P10","Q10","Q11","Q21~23","Q26","Q27","R11","R21~24","R26","S21~23"]
ProjectName={"Q26":"ScottyRr", "Q27":"Scotty", "R24":"Worf", "R26":"Riker"}
...
NotRemovefilerule=["Note", "note", "History", "How to Flash", "AMT_CFG"] # Priority over "Removefilerule"

Removefilerule=["DCI.7z", ".cer", ".pfx", ".pvk", ".xlsm", r"Pvt.bin", r"metainfo.xml", r"Build.Log",
...
if __name__ == '__main__':
    ...
    #=================Script Start==========================================================
    print("Make Release Pkg Script   4.3.9".center(90, "="))
    print("Input Information".center(90, "="))
    print("This script for makeing release pkg.\nNow can use Intel G4, G5, G6  AMD G4 G5 platform.\n")
    ...
    for Project in ProcessProjectList:
        temp = []
        #======For Intel
        if (Platform_Flag(Project) == "Intel G3") or (Platform_Flag(Project) == "Intel G4") or (Platform_Flag(Project) == "Intel G5") or (Platform_Flag(Project) == "Intel G6"):
    ...
    for OldProcPkg in OldProcPkgInfo:
        #======For Intel
        if (Platform_Flag(OldProcPkg) == "Intel G3") or (Platform_Flag(OldProcPkg) == "Intel G4") or (Platform_Flag(OldProcPkg) == "Intel G5") or (Platform_Flag(OldProcPkg) == "Intel G6"):
        ...
        #======For AMD
            ...
            for Proc in range(len(NeedProcOldPkg)):
                if (Platform_Flag(OldProcPkg) == "R26"):# 78 AMD 78 R26 R24
                ...
                if Platform_Flag(OldProcPkg) == "R24":
                    ...
                    NewProcPkgInfo[Proc][2] = NewVersion_AMD # ex:['Worf', 'R24','NewVersion(02.07.03)']
            if NewBuildID == "0000":
                NewBuildID = ""
            if not (NewBuildID == "" or NewBuildID == "0000"):
                NewProcPkgInfo[Proc].append(NewBuildID)
    ...
    for PkgInfo in range(len(NewProcPkgInfo)):
        #======For AMD Start
        if Platform_Flag(OldProcPkgInfo) == "AMD" or Platform_Flag(OldProcPkgInfo) == "R26": # ex.['Q26', '01.04.01']=>['ScottyRr', 'ScottyRr', 'Q26', '010401']
    ...
    for NProc in NewProcPkgInfo:# Pkg Modify Update Version
        #======For Intel
        if (Platform_Flag(NProc) == "Intel G3") or (Platform_Flag(NProc) == "Intel G4") or (Platform_Flag(NProc) == "Intel G5") or (Platform_Flag(NProc) == "Intel G6"):
                ...
                if len(ReleaseNote_docx) == 1:
                    ...
                    if (NewBuildID == "" or NewBuildID == "0000"):
                        ...
                        if os.path.isfile(ReleaseName + NewVersion + ".xlsm"):
                            ModifyReleaseNote(NProc, ReleaseName + NewVersion + ".xlsm", BiosBuildDate, BiosBinaryCRC32, NewVersion, NewBuildID)
                        ...
                        if os.path.isfile(ReleaseName + NewVersion + "_" + NewBuildID + ".xlsm"):
                            ModifyReleaseNote(NProc, ReleaseName + NewVersion + "_" + NewBuildID + ".xlsm", BiosBuildDate, BiosBinaryCRC32, NewVersion, NewBuildID)
                ...
                if len(ReleaseNote_docx) == 1:
                    ...
                    if (NewBuildID == "" or NewBuildID == "0000"):
                        ...
                        if os.path.isfile(ReleaseName + "Note_" + NewVersion + ".xlsm"):
                            ModifyReleaseNote(NProc, ReleaseName + "Note_" + NewVersion + ".xlsm", BiosBuildDate, BiosBinaryCRC32, NewVersion, NewBuildID)
                        ...
                        if os.path.isfile(ReleaseName + "Note_" + NewVersion + "_" + NewBuildID + ".xlsm"):
                            ModifyReleaseNote(NProc, ReleaseName+"Note_"+NewVersion+"_"+NewBuildID+".xlsm", BiosBuildDate, BiosBinaryCRC32, NewVersion, NewBuildID)
    ...
    for Fv in Matchfolderlist:
        for NProc in NewProcPkgInfo:
            ...
            #======For AMD
                ...
                if (Platform_Flag(NProc) == "R26") or (Platform_Flag(NProc) == "R24") or (Platform_Flag(NProc) == "AMD"):
                    if (Fv.split("_")[1] == NProc[0]) or (Fv.split("_")[1] == NProc[1]):
                        if os.path.isdir(".\\" + Fv):
                         ...
                         if os.path.isfile(Path + "\\" + Boardversion + ".bin") or os.path.isfile(Path + "\\" + Boardversion + "_16.bin"):# For 16MB BIOS
                            if os.path.isfile(Path + "\\" + Boardversion + ".inf") and os.path.isfile(Path + "\\" + Boardversion[:3] + ".xml"):
    ...
    try:
        for NProc in NewProcPkgInfo:
            #======For Intel G5&G6
            if ((Platform_Flag(NProc) == "Intel G5") or (Platform_Flag(NProc) == "Intel G6")) and os.path.isdir(".\\" + ("_").join(NProc) + "\\Capsule\\CCG5\\CCG5C"):
    ...
    if (Platform_Flag(OldProcPkgInfo) == "Intel G3") or (Platform_Flag(OldProcPkgInfo) == "Intel G4") or \
       (Platform_Flag(OldProcPkgInfo) == "Intel G5") or (Platform_Flag(OldProcPkgInfo) == "Intel G6"):
