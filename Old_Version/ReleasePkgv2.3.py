		...
		if BoardID[Project] in Processlist:# Find process BoardID
			if BoardID[Project]==Boradname[1]:# If Fv folder boardID match
				...
				if not (os.path.isdir(NewVersionPath)):# Check NewVersion Folder Exist
					if (os.path.isdir(OldVersionPath)):# Check OldVersion Folder Exist
						print("Start Copy "+OldVersionPkg+" to "+NewVersionPkg+", Please wait.....")
						shutil.copytree(OldVersionPath, NewVersionPath)# Copy to new Pkg
						print("Copy Pkg "+OldVersionPkg+" to "+NewVersionPkg+" succeeded.")
					else:
						if sys.version > '3' and OldBuildID==0:# If OldVersion pkg have BuildID
							OldBuildID=input("If your "+Projectlist[Project]+" OldVersion folder have BuildID, Please input:");# ex: 0001
						elif OldBuildID==0:
							OldBuildID=raw_input("If your "+Projectlist[Project]+" OldVersion folder have BuildID, Please input:");
						if (os.path.isdir(OldVersionPath+"_"+OldBuildID)):
							print("Start Copy "+OldVersionPkg+"_"+OldBuildID+" to "+NewVersionPkg+", Please wait.....")
							shutil.copytree(OldVersionPath+"_"+OldBuildID, NewVersionPath)# Copy to new Pkg
							print("Copy Pkg "+OldVersionPkg+"_"+OldBuildID+" to "+NewVersionPkg+" succeeded.")
						else:
							print(Projectlist[Project]+" NewVersion:"+NewVersion+" & OldVersion:"+OldVersion+"_"+OldBuildID+" Pkg folder both can't find, Please check.")
