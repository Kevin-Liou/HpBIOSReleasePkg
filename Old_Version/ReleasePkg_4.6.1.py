...
BoardID=["P10","Q10~11","Q21~23","Q26~27","R11","R21~24","R26","S10~11","S21~23","T21~22","S25~S29"]
ProjectName={"Q26":"ScottyRr", "Q27":"Scotty", "R24":"Worf", "R26":"Riker", "S25":"DoppioPco", "S27":"DoppioRn", "S29":"CubanoRn"}
...
NotRemovefilerule=["Note", "note", "History", "How to Flash", "AMT_CFG", "logo", "sign.bin", "HPSignME"] # Priority over "Removefilerule"
...
Removefilerule=["DCI..+", ".cer", ".pfx", ".pvk", ".xlsm", ".log", r"Pvt.bin", r"metainfo.xml", r"Build.Log",
                ...
                r"P00\w{3}-\w{3}.zip", r"TBT_RTD3.+", r"HP_\w+_\w+_\w+_\w+_\w+_\w+_\d+.+",
...
if __name__ == '__main__':
    ...
    #=================Script Start==========================================================
    print("Make Release Pkg Script   4.6.1".center(90, "="))
    print("Input Information".center(90, "="))
    print("This script for makeing release pkg.\nNow can use Intel G4, G5, G6, G8  AMD G4 G5 platform.\n")
    ...
    for Project in ProcessProjectList:
        ...
        if (Platform_Flag(Project) == "Intel G3") or (Platform_Flag(Project) == "Intel G4") or \
           ...
           (Platform_Flag(Project) == "Intel G8"):
        ...
        #======For AMD
            ...
            if (Platform_Flag(Project) == "R26") or (Platform_Flag(Project) == "S25") or \
               (Platform_Flag(Project) == "S27") or (Platform_Flag(Project) == "S29"):# 78 AMD 78 R26 S25~S29
    ...
    for OldProcPkg in OldProcPkgInfo:
        ...
        if (Platform_Flag(OldProcPkg) == "Intel G3") or (Platform_Flag(OldProcPkg) == "Intel G4") or \
           ...
           (Platform_Flag(OldProcPkg) == "Intel G8"):
        ...
        #======For AMD
            ...
            for Proc in range(len(NeedProcOldPkg)):
                if (Platform_Flag(OldProcPkg) == "R26") or (Platform_Flag(OldProcPkg) == "S25") or \
                   (Platform_Flag(OldProcPkg) == "S27") or (Platform_Flag(OldProcPkg) == "S29"): # 78 AMD 78 R26 R24 S25~S29
    ...
    for PkgInfo in range(len(NewProcPkgInfo)):
        #======For AMD Start
        if (Platform_Flag(OldProcPkgInfo) == "AMD") or (Platform_Flag(OldProcPkgInfo) == "R26") or \
           (Platform_Flag(OldProcPkgInfo) == "S25") or (Platform_Flag(OldProcPkgInfo) == "S27") or \
           (Platform_Flag(OldProcPkgInfo) == "S29"):# ex.['Q26', '01.04.01']=>['ScottyRr', 'ScottyRr', 'Q26', '010401']
    ...
    for PkgInfo in range(len(NewProcPkgInfo)):
        ...
        if (Platform_Flag(OldProcPkgInfo) == "AMD") or (Platform_Flag(OldProcPkgInfo) == "R26") or \
           (Platform_Flag(OldProcPkgInfo) == "S25") or (Platform_Flag(OldProcPkgInfo) == "S27")  or \
           (Platform_Flag(OldProcPkgInfo) == "S29"): # ['ScottyRr', 'ScottyRr', 'Q26', '010401']=>['Q26', '01.04.01']
    ...
    for Fv in Matchfolderlist:
        for NProc in NewProcPkgInfo:
            ...
            #======For AMD
                ...
                if (Platform_Flag(NProc) == "R26") or (Platform_Flag(NProc) == "R24") or (Platform_Flag(NProc) == "AMD") or \
                   (Platform_Flag(NProc) == "S25") or (Platform_Flag(NProc) == "S27") or (Platform_Flag(NProc) == "S29"):
    ...
    try:
        for NProc in NewProcPkgInfo:
            ...
            if ((Platform_Flag(NProc) == "Intel G5") or (Platform_Flag(NProc) == "Intel G6") or \
                (Platform_Flag(NProc) == "Intel G8")) and os.path.isdir(".\\" + ("_").join(NProc) + "\\Capsule\\Windows\\Combined FW Image (BIOS, ME, PD)"):
    ...
    if (Platform_Flag(OldProcPkgInfo) == "Intel G3") or (Platform_Flag(OldProcPkgInfo) == "Intel G4") or \
       ...
       (Platform_Flag(OldProcPkgInfo) == "Intel G8"):
